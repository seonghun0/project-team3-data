...
def train_recommender_system(): 
    ...
    with open('similarity_genre.pickle', "wb") as f:
        pickle.dump(similarity_genre, f, protocol=4)

    with open('movie_data.pickle', "wb") as f:
        pickle.dump(data, f)


def recommend_movies(target_movie_index, similarity_genre, data, top):
    # 특정영화 정보 뽑아내기
...
if __name__ == "__main__":
    ...
    if not os.path.exists("similarity_genre.pickle") or not os.path.exists("movie_data.pickle"):
        train_recommender_system()

    with open('similarity_genre.pickle', "rb") as f:
    ...
    recommended_movies = recommend_movies(target_movie_index, similarity_genre, data, 30)
